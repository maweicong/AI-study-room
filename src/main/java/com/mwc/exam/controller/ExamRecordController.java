...
private ExamService examService;
    ...
    public Result<Page<ExamRecord>> getExamRecords(
        ...
        examRecordService.examRecord(pageObj, studentName, status, startDate, endDate);
    ...
    public Result<ExamRecord> getExamRecordById(
        ...
        ExamRecord byPaperId = examService.customGetExamRecordById(id);
        return Result.success(byPaperId);
    }
    ...
    public Result<Void> deleteExamRecord(
            @Parameter(description = "考试记录ID") @PathVariable Integer id) {
        examService.custRemoveById(id);
        log.info("删除考试记录，参数：{}", id);
    ...
    public Result<List<ExamRankingVO>> getExamRanking(
        ...
        // 使用优化的查询方法，避免N+1查询问题
        List<ExamRankingVO> examRanking = examRecordService.getExamRanking(paperId, limit);
        log.info("获取考试排行榜，参数：{}", examRanking);
        return Result.success(examRanking);
    }
} 