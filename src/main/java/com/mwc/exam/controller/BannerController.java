...
import com.mwc.exam.entity.BaseEntity;
...
public class BannerController {
    ...
    public Result<List<Banner>> getAllBanners() {
        ...
        queryWrapper.orderByAsc(Banner::getSortOrder);
        List<Banner> list = bannerService.list(queryWrapper);
        log.info("获取后台轮播图列表：{}", list);
        return Result.success(list,"查询所有轮播图数据成功");
    }
    ...
    public Result<String> updateBanner(@RequestBody Banner banner) {
        ...
        log.info("id为{}的轮播图已更新：{}",banner.getId(), banner);
    ...
    public Result<String> deleteBanner(@Parameter(description = "轮播图ID") @PathVariable Long id) {
        ...
        boolean delete = bannerService.remove(queryWrapper);
        if ( delete){
    ...
    public Result<String> toggleBannerStatus(
        ...
        queryWrapper.eq(Banner::getId, id);
        queryWrapper.set(Banner::getIsActive, isActive);
        boolean update = bannerService.update(queryWrapper);
        log.info("id为{}切换轮播图状态：{}",id, update);
        if (update) {
