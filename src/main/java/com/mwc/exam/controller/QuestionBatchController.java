...
public class QuestionBatchController {
    ...
    public ResponseEntity<byte[]> downloadTemplate() throws IOException {
         ...
         * 生成Excel模板文件并返回给前端下载
         *
         * @return ResponseEntity<byte[]> 包含Excel模板文件的响应实体，包含文件字节流和下载头部信息
         */
        //生成模板excel对应的字节数组
        byte[] templateBytes = ExcelUtil.generateTemplate();
        //数据封装到ResponseEntity
        ResponseEntity<byte[]> body = ResponseEntity.ok()// 设置状态码为200（成功）
                .header("Content-Disposition", "attachment; filename=excel_template.xlsx")// 响应头：告诉浏览器“这是附件，文件名是xxx”
                .contentType(MediaType.APPLICATION_OCTET_STREAM)// 响应头：说明数据是二进制流（文件类型）
                .body(templateBytes);// 设置要返回的核心数据（模板字节数组）
        return body;
    }
    ...
    public Result<List<QuestionImportVo>> generateQuestionsByAi(
        ...
        List<QuestionImportVo> questions = kimiAiService.generateQuestionsByAi(request);
        log.info("使用AI生成的题目列表：{},希望生成数量为{},最终生成数量为{}", questions,request.getCount(), questions.size());
       return Result.success(questions);
    }
